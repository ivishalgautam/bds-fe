...
const fetchTodos = () => {
  return http().get(`${endpoints.todos.getAll}?type=to_do`);
};

const deleteItem = async (itemId) => {
  await http().delete(`${endpoints.todos.getAll}/${itemId}`);
};

const createItem = async (newItem) => {
  await http().post(endpoints.todos.getAll, newItem);
};

const updateItem = async (itemId, updatedItem) => {
  await http().put(`${endpoints.todos.getAll}/${itemId}`, updatedItem);
};
      ...
      <div className="grid grid-cols-2 gap-8">
        ...
        >
          <div className="flex flex-col items-center justify-center space-y-4">
            ...
            <h3 className="font-bold">Add New Todo</h3>
          </div>
        </div>
        {data.map((item) => (
          <div
