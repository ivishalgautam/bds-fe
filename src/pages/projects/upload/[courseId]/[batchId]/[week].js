...
const postProject = async (data) => {
  return await http().post(endpoints.projects.uploadProject, data);
};

export default function UploadProjecct() {
  const [fileName, setFileName] = useState("Click to upload project");
  const [file, setFile] = useState(null);
  const router = useRouter();
  const { courseId, batchId, week } = router.query;
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!file) {
      return toast.error("Please select your project!");
    }
